...
class Graph:
    ...
    def ucs(self, start, goal):
        ...
        print(f"\n{'Iter':<10} | {'Frontier (cost:node)':<35} | {'Explored'}")
        print("-" * 70)

        while frontier:
            frontier_view = [(c, n) for c, n, _ in sorted(frontier)]
            print(f"{itr:<10} | {str(frontier_view):<35} | {sorted(explored)}")

            cost, node, path = heapq.heappop(frontier)  # pop lowest cost
            ...
            for nb, weight in self.vertices[node]:
                if nb not in explored:
                    heapq.heappush(frontier, (cost + weight, nb, path + [nb]))
